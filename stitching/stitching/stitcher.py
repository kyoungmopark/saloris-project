...
class Stitcher:
    ...
    def stitch(self, img_names):
        ...
        if self.cachedF is None: 
            features = self.find_features(imgs)
            matches = self.match_features(features)
            self.cachedF = features 
            self.cachedM = matches
            
        imgs, features, matches = self.subset(imgs, self.cachedF, self.cachedM)
        if self.cachedC is None: 
            cameras = self.estimate_camera_parameters(features, matches)
            cameras = self.refine_camera_parameters(features, matches, cameras)
            cameras = self.perform_wave_correction(cameras)
            self.cachedC = cameras 

        self.estimate_scale(self.cachedC)
        imgs = self.resize_low_resolution(imgs)
        imgs, masks, corners, sizes = self.warp_low_resolution(imgs, self.cachedC)
        ...
        imgs, masks, corners, sizes = self.warp_final_resolution(imgs, self.cachedC)
