...
async def get_stream_video(width, height,parking_lot,camera_index):
    model = torch.hub.load('ultralytics/yolov5', 'yolov5s', pretrained=True)
    n = len(camera_list)
    cams = [None for _ in range(n)]   
    tracked_objects = [None for _ in range(n)]   
    trackers=[None for _ in range(n)]   
    for idx, camera in enumerate(camera_list):
        trackers[idx] = DeepSort(r"C:\cctvproject\New\DeepSORT_YOLOv5_Pytorch\deep_sort\deep\checkpoint\ckpt.t7")
        cams[idx] = cv2.VideoCapture(camera)
    while True:
        ...
        for i in range(n):
            ...
            tracked_objects[i]= trackers[i].update(boxes, class_ids, frame)
            frames[i] = frame
            
            for d in tracked_objects[i]:
                xmin, ymin, xmax, ymax, id,vx,vy = d
                center_x = (xmin + xmax) / 2
                center_y = (ymin + ymax) / 2
                if i==camera_index:
                    cv2.rectangle(frames[camera_index], (xmin ,ymin), (xmax, ymax), (0, 255, 0), 2)
                    cv2.putText(frames[camera_index], str(id), (xmin, ymin - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                if i in parking_lot:
                    for spot in parking_lot[i]:
        ...
        for idx, spot in enumerate(parking_lot.get(camera_index, [])):
            ...
            cv2.line(frames[camera_index], lefttop, righttop, (255, 0, 0), 2)
            cv2.line(frames[camera_index], righttop, rightbottom, (255, 0, 0), 2)
            cv2.line(frames[camera_index], rightbottom, leftbottom, (255, 0, 0), 2)
            cv2.line(frames[camera_index], leftbottom, lefttop, (255, 0, 0), 2)
            cv2.putText(frames[camera_index], str(spot.parkingindex), (lefttop[0] + 10, lefttop[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        end_time = time.time()
        processing_time = end_time - start_time
        print(f"Processing speed: {processing_time}")
...
@app.get("/video/{cam_id}/{width}/{height}")
async def main(cam_id: int, width: int, height: int):
    try:
        return StreamingResponse(get_stream_video(width, height,parking_lot,cam_id), media_type="multipart/x-mixed-replace; boundary=frame")
    except IndexError:
